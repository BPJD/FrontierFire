...
Shader "Triplanar/BuiltIn/Simple Top Middle Bottom"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            // Chose between local and global space
            float3 localPos = mul(unity_WorldToObject, float4(IN.worldPos, 1.0)).xyz;
            float3 pos = lerp(IN.worldPos, localPos, _LocalSpace);
            // Applying texture scale a la TRANSFORM_TEX macro.
            float2 uvX = pos.zy * _MainTex_ST.xy;
            float2 uvYPlus = pos.xz * _TopTex_ST.xy;
            float2 uvYMinus = pos.xz * _BottomTex_ST.xy;
            float2 uvZ = pos.xy * _MainTex_ST.xy;
